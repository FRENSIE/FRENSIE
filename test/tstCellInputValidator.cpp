...
// FACEMC Includes
#include "CellInputValidator.hpp"
#include "SurfaceInputValidator.hpp"
...
TEUCHOS_UNIT_TEST( CellInputValidator, validateCell )
{
  ...
  TEST_NOTHROW( FACEMC::CellInputValidator::validateCell( valid_cell,
							  valid_cell.name(),
							  surface_names ) );
  ...
  TEST_THROW( FACEMC::CellInputValidator::validateCell( invalid_cell_1,
							invalid_cell_1.name(),
							surface_names ),
  ...
  TEST_THROW( FACEMC::CellInputValidator::validateCell( invalid_cell_2,
							invalid_cell_2.name(),
							surface_names ),
	      std::invalid_argument );
  ...
  TEST_THROW( FACEMC::CellInputValidator::validateCell( copy_cell,
							copy_cell.name(),
							surface_names ),
	      std::invalid_argument );

  FACEMC::CellInputValidator::clearCellNames();
  FACEMC::CellInputValidator::clearMaterialNames();
}
...
TEUCHOS_UNIT_TEST( CellInputValidator, validateAllCells )
{
  ...
  FACEMC::SurfaceInputValidator::validateAllSurfaces( 
					geometry.sublist( "surfaces", true ) );

  // Get the surface names
  std::set<std::string> surface_names =
    FACEMC::SurfaceInputValidator::getSurfaceNames();

  // Validate the cells
  TEST_NOTHROW( FACEMC::CellInputValidator::validateAllCells(
...
TEUCHOS_UNIT_TEST( CellInputValidator, getCellNames )
{
  std::set<std::string> cell_names = 
    FACEMC::CellInputValidator::getCellNames();
...
TEUCHOS_UNIT_TEST( CellInputValidator, getMaterialNames )
{
  std::set<std::string> material_names = 
    FACEMC::CellInputValidator::getMaterialNames();
