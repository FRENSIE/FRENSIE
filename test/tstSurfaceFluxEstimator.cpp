...
// FACEMC Includes
#include "SurfaceFluxEstimator.hpp"
...
template<typename SurfaceFluxEstimator>
void initializeSurfaceFluxEstimator( 
				Teuchos::RCP<SurfaceFluxEstimator>& estimator )
{  
  // Set the entity ids
  Teuchos::Array<FACEMC::Traits::ModuleTraits::InternalSurfaceHandle> 
    surface_ids( 2 );
  surface_ids[0] = 0;
  surface_ids[1] = 1;
  ...
  estimator.reset( new SurfaceFluxEstimator( 0ull, 
...
{
  typedef FACEMC::SurfaceFluxEstimator<ContributionMultiplierPolicy>
    SurfaceFluxEstimator;

  Teuchos::RCP<SurfaceFluxEstimator> estimator;
  initializeSurfaceFluxEstimator<SurfaceFluxEstimator>( estimator );
...
UNIT_TEST_INSTANTIATION( SurfaceFluxEstimator, 
			 addPartialHistoryContribution );
