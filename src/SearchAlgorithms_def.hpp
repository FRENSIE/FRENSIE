				     ...
				     const typename TupleComparePolicy<typename Iterator::value_type,member>::tupleMemberType value )
{
  ...
  testPrecondition( (value >= TupleComparePolicy<typename Iterator::value_type,member>::value( *start )) );
  testPrecondition( (value <= TupleComparePolicy<typename Iterator::value_type,member>::value( *(end-1) )) );

  // Remember the end iterator for the Postcondition check
  remember( Iterator invalid = end );
  
  typename Iterator::difference_type distance = std::distance( start, end );
  
  while( distance > 1 )
  {
    if( value >= TupleComparePolicy<typename Iterator::value_type,member>::value( *(start+distance/2) ) )
      start += distance/2;
    else
				   ...
				   const typename TupleComparePolicy<typename Iterator::value_type,member>::tupleMemberType value )
{
  ...
  if( distance == 1 )
  {
    if( value > TupleComparePolicy<typename Iterator::value_type,member>::value( *(start) ) )
      start = end;
  }
