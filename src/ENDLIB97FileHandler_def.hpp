 ...
 * only compile if the template parameter is a FACEMC tuple struct (either
 * FACEMC::Pair, FACEMC::Trip or FACEMC::Quad). 
 * \tparam T A FACEMC tuple struct (either FACEMC::Pair, FACEMC::Trip or
 * FACEMC::Quad) that is used to store a data point from the table. 
 ...
 * \tparam T A FACEMC tuple struct (either FACEMC::Trip or FACEMC::Quad) that
 ...
 * raw table data is stored in an array of tuple structs. This function will
 * only compile if the template parameter is a FACEMC tuple struct (only
 * FACEMC::Quad).
 * \tparam T a FACEMC tuple struct (only FACEMC::Quad) that is used to store
