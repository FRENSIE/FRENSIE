//---------------------------------------------------------------------------//
//!
//! \file   Utility_GaussKronrodBasicIntegrator_def.hpp
//! \author Luke Kersting
//! \brief  Gauss-Kronrod integrator
//!
//---------------------------------------------------------------------------//

#ifndef UTILITY_GAUSS_KRONROD_BASIC_INTEGRATOR_DEF_HPP
#define UTILITY_GAUSS_KRONROD_BASIC_INTEGRATOR_DEF_HPP
...
void GaussKronrodBasicIntegrator::integrateAdaptively(
						 Functor& integrand, 
						 double lower_limit, 
						 double upper_limit,
						 double& result,
...
void GaussKronrodBasicIntegrator::integrateAdaptivelyWynnEpsilon( 
    Functor& integrand,
 ...
 */ 
template<int Points, typename Functor>
void GaussKronrodBasicIntegrator::integrateWithPointRule(
            Functor& integrand, 
            double lower_limit, 
            double upper_limit,
            long double& result,
            ...
            double& result_asc ) const
{
  ...
  testStaticPrecondition( GaussKronrodBasicQuadratureSetTraits<Points>::valid_rule );
  ...
  if( lower_limit < upper_limit )
  {
    ...
    int number_of_weights =
        GaussKronrodBasicQuadratureSetTraits<Points>::kronrod_weights.size();
    ...
    for ( int j = 0; j < number_of_weights-1; j++ )
      {  
        calculateQuadratureIntegrandValuesAtAbscissa( 
            integrand, 
            GaussKronrodBasicQuadratureSetTraits<Points>::kronrod_abscissae[j],
            half_length,
            midpoint,
        ...
        kronrod_result += 
            GaussKronrodBasicQuadratureSetTraits<Points>::kronrod_weights[j]*integrand_values_sum[j];

        result_abs += GaussKronrodBasicQuadratureSetTraits<Points>::kronrod_weights[j]*( 
    ...
    long double kronrod_result_last_weight = integrand_midpoint*
        GaussKronrodBasicQuadratureSetTraits<Points>::kronrod_weights[number_of_weights-1];
    ...
    for ( int j = 0; j < number_of_weights - 1; j++ )
      {  
        result_asc += GaussKronrodBasicQuadratureSetTraits<Points>::kronrod_weights[j]*
    ...
    result_asc += GaussKronrodBasicQuadratureSetTraits<Points>::kronrod_weights[number_of_weights-1]*
    ...
    for ( int j = 0; j < (number_of_weights-1)/2; j++ )
      {
        ...
        gauss_result += integrand_values_sum[jj]*
            GaussKronrodBasicQuadratureSetTraits<Points>::gauss_weights[j];
      };
    ...
    if ( number_of_weights % 2 == 0 )
    {
      gauss_result += integrand_midpoint*
        GaussKronrodBasicQuadratureSetTraits<Points>::gauss_weights[number_of_weights/2 - 1];
    }
...
template<int Points>
inline bool GaussKronrodBasicIntegrator::subintervalTooSmall( 
...
template<typename Functor>
void GaussKronrodBasicIntegrator::calculateQuadratureIntegrandValuesAtAbscissa( 
    Functor& integrand, 
    double abscissa,
    double half_length,
    double midpoint,
...
void GaussKronrodBasicIntegrator::bisectAndIntegrateBinInterval( 
    Functor& integrand, 
    const Bin& bin,
    Bin& bin_1,
    Bin& bin_2,
...
#endif // end UTILITY_GAUSS_KRONROD_BASIC_INTEGRATOR_DEF_HPP

//---------------------------------------------------------------------------//
// end Utility_GaussKronrodBasicIntegrator_def.hpp
//---------------------------------------------------------------------------//
