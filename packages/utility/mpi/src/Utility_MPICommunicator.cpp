...
MPICommunicator::split( int MPI_ENABLED_PARAMETER(color) ) const
{
  ...
  return MPICommunicator::createNewComm( sub_comm );
#else
  return std::shared_ptr<const Communicator>();
#endif // end HAVE_FRENSIE_MPI
}
...
std::shared_ptr<const Communicator> MPICommunicator::createNewComm( const boost::mpi::communicator& comm )
{
  return std::shared_ptr<const Communicator>( new MPICommunicator( comm ) );
}
#endif // end HAVE_FRENSIE_MPI
