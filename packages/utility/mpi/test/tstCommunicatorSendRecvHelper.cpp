//---------------------------------------------------------------------------//
//!
//! \file   tstCommunicatorSendRecvHelper.cpp
//! \author Alex Robinson
//! \brief  Communicator send/receive helper function unit tests
//!
//---------------------------------------------------------------------------//
...
FRENSIE_UNIT_TEST_TEMPLATE( CommunicatorHelpers, send_recv_basic, BasicTypes )
{
  ...
  if( comm->size() > 1 )
  {
    ...
    std::vector<int> number_of_values( {10, 8, 4} );
    ...
    else
    {
      ...
      {
        // Raw data receive
        T* values_to_receive = new T[number_of_values[raw_data_send_tag]];
        Utility::ArrayView<T> view_of_values_to_receive( values_to_receive, values_to_receive+number_of_values[raw_data_send_tag] );

        T* expected_values_to_receive = new T[number_of_values[raw_data_send_tag]];
        Utility::ArrayView<const T> view_of_expected_values_to_receive;

        {
          Utility::ArrayView<T> tmp_view_of_expected_values_to_receive( expected_values_to_receive, expected_values_to_receive+number_of_values[raw_data_send_tag] );
          tmp_view_of_expected_values_to_receive.fill( value );
        ...
        Utility::Communicator::Status comm_status = 
          Utility::receive( *comm, i, raw_data_send_tag, view_of_values_to_receive );
        ...
        // View data receive
        view_of_values_to_receive.fill( initializeValue( T(), 0 ) );

        comm_status = Utility::receive( *comm, i, view_data_send_tag, view_of_values_to_receive( 1, number_of_values[view_data_send_tag] ) );
        ...
        FRENSIE_CHECK_EQUAL( view_of_values_to_receive( 1, number_of_values[view_data_send_tag] ),
                             view_of_expected_values_to_receive( 1, number_of_values[view_data_send_tag] ) );

        // Initializer list temporary receive
        view_of_values_to_receive.fill( initializeValue( T(), 0 ) );

        comm_status = Utility::receive( *comm, i, init_list_data_send_tag, view_of_values_to_receive( 0, number_of_values[init_list_data_send_tag] ) );
        ...
        FRENSIE_CHECK_EQUAL( view_of_values_to_receive( 0, number_of_values[init_list_data_send_tag] ),
                             view_of_expected_values_to_receive( 0, number_of_values[init_list_data_send_tag] ) );

        // Free raw data
        delete[] values_to_receive;
        delete[] expected_values_to_receive;
      }
    }
  }
  else
  {
    ...
    FRENSIE_CHECK_THROW( Utility::send( *comm, 0, 0, Utility::ArrayView<const T>(&dummy_data, &dummy_data+1) ),
                         Utility::InvalidCommunicator );
    FRENSIE_CHECK_THROW( Utility::receive( *comm, 0, 0, Utility::ArrayView<T>(&dummy_data, &dummy_data+1) ),
                         Utility::InvalidCommunicator );
  }
}
...
FRENSIE_UNIT_TEST_TEMPLATE( CommunicatorHelpers, send_recv, ContainerTypes )
{
  ...
  if( comm->size() > 1 )
  {
    ...
    if( comm->rank() > 0 )
    {
      ...
      Utility::send( *comm, 0, single_container_tag, containers_to_send.front() );
      Utility::send( *comm, 0, multiple_containers_tag, Utility::arrayView(containers_to_send) );
    }
    else
    {
      std::vector<Container>
      ...
      for( int i = 1; i < comm->size(); ++i )
      {
        ...
        Utility::Communicator::Status comm_status =
          Utility::receive( *comm, i, single_container_tag, containers_to_receive.front() );
        
        FRENSIE_REQUIRE( comm_status.hasMessageDetails() );
        FRENSIE_CHECK_EQUAL( comm_status.source(), i );
        FRENSIE_CHECK_EQUAL( comm_status.tag(), single_container_tag );
        FRENSIE_CHECK_EQUAL( comm_status.count(), 1 );
        FRENSIE_CHECK_EQUAL( containers_to_receive.front(),
                             expected_containers_to_receive.front() );

        comm_status = Utility::receive( *comm, i, multiple_containers_tag, Utility::arrayView(containers_to_receive) );

        FRENSIE_REQUIRE( comm_status.hasMessageDetails() );
        FRENSIE_CHECK_EQUAL( comm_status.source(), i );
        FRENSIE_CHECK_EQUAL( comm_status.tag(), multiple_containers_tag );
        FRENSIE_CHECK_EQUAL( comm_status.count(), 3 );
...
//---------------------------------------------------------------------------//
// end tstCommunicatorSendRecvHelper.cpp
//---------------------------------------------------------------------------//
