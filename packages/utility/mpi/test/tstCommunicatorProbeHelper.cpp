//---------------------------------------------------------------------------//
//!
//! \file   tstCommunicatorProbeHelper.cpp
//! \author Alex Robinson
//! \brief  Communicator probe helper function unit tests
//!
//---------------------------------------------------------------------------//
...
FRENSIE_UNIT_TEST_TEMPLATE( Communicator, probe, Types )
{
  ...
  if( comm->size() > 1 )
  {
    ...
    else
    {
      for( int i = 1; i < comm->size(); ++i )
      {
        ...
        Utility::Communicator::Status comm_status =
          Utility::probe<std::array<T,10> >( *comm, i, container_data_send_tag );
  ...
  else
  {
    ...
    FRENSIE_CHECK_THROW( (Utility::probe<std::array<T,10> >( *comm, 0, 0 )),
                         Utility::InvalidCommunicator );
  }
}
...
//---------------------------------------------------------------------------//
// Check that a blocking probe can be conducted
FRENSIE_UNIT_TEST_TEMPLATE( Communicator, probe_any_source, Types )
{
  FETCH_TEMPLATE_PARAM( 0, T );
  ...
  if( comm->size() > 1 )
  {
    ...
    else
    {
      ...
      {
        Utility::Communicator::Status comm_status =
          Utility::probe<std::array<T,10> >( *comm, array_data_send_tag );
...
//---------------------------------------------------------------------------//
// Check that a blocking probe can be conducted
FRENSIE_UNIT_TEST_TEMPLATE( Communicator, probe_any_source_any_tag, Types )
{
  FETCH_TEMPLATE_PARAM( 0, T );
  ...
  else
  {
    FRENSIE_CHECK_THROW( Utility::probe<T>( *comm ),
                         Utility::InvalidCommunicator );
    FRENSIE_CHECK_THROW( (Utility::probe<std::array<T,10> >( *comm )),
                         Utility::InvalidCommunicator );
  }
}

//---------------------------------------------------------------------------//
// end tstCommunicatorProbeHelper.cpp
//---------------------------------------------------------------------------//
