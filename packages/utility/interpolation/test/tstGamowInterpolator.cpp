...
TEUCHOS_UNIT_TEST( GamowInterpolator, getExothermicInstance )
{
  ...
  TEST_ASSERT( exothermic_instance.get() != NULL );
}

//---------------------------------------------------------------------------//
// Check that an instance of the interpolator can be retrieved
TEUCHOS_UNIT_TEST( GamowUnitAwareInterplator, getExothermicInstance )
{
...
TEUCHOS_UNIT_TEST( GamowInterpolator, getInterpolationType )
{
...
//---------------------------------------------------------------------------//
// Check that the interpolation type can be returned
TEUCHOS_UNIT_TEST( GamowUnitAwareInterpolator, getInterpolationType )
{
...
//---------------------------------------------------------------------------//
// Check that the interpolation type can be returned
TEUCHOS_UNIT_TEST( GamowInterpolator, isIndepVarInValidRange )
{
  std::shared_ptr<const Utility::Interpolator<double> > interpolator =
    Utility::GamowInterpolator<double>::getExothermicInstance();
  ...
  TEST_ASSERT( !interpolator->isIndepVarInValidRange(
				       -std::numeric_limits<double>::max() ) );
  TEST_ASSERT( !interpolator->isIndepVarInValidRange( 1.0 ) );
  TEST_ASSERT( interpolator->isIndepVarInValidRange( 1.0+ 1e-12 ) );
  TEST_ASSERT( interpolator->isIndepVarInValidRange( 
					std::numeric_limits<double>::max() ) );
}
...
//---------------------------------------------------------------------------//
// Check that the interpolation type can be returned
TEUCHOS_UNIT_TEST( GamowUnitAwareInterpolator, isIndepVarInValidRange )
{
  std::shared_ptr<const Utility::UnitAwareInterpolator<MegaElectronVolt,Barn,double> > interpolator = 
    Utility::GamowUnitAwareInterpolator<MegaElectronVolt,Barn,double>::getExothermicInstance();
  ...
  TEST_ASSERT( !interpolator->isIndepVarInValidRange(
                                   -std::numeric_limits<double>::max()*MeV ) );
  TEST_ASSERT( !interpolator->isIndepVarInValidRange( 1.0*MeV ) );
  TEST_ASSERT( interpolator->isIndepVarInValidRange( (1.0+1e-12)*MeV ) );
  TEST_ASSERT( interpolator->isIndepVarInValidRange( 
                                    std::numeric_limits<double>::max()*MeV ) );
}
...
TEUCHOS_UNIT_TEST( GamowInterpolator, isDepVarInValidRange )
{
  std::shared_ptr<const Utility::Interpolator<double> > interpolator =
    Utility::GamowInterpolator<double>::getExothermicInstance();
...
//---------------------------------------------------------------------------//
// Check that the validity of a dependent variable can be tested
TEUCHOS_UNIT_TEST( GamowUnitAwareInterpolator, isDepVarInValidRange )
{
  std::shared_ptr<const Utility::UnitAwareInterpolator<MegaElectronVolt,Barn,double> > interpolator = 
    Utility::GamowUnitAwareInterpolator<MegaElectronVolt,Barn,double>::getExothermicInstance();
...
TEUCHOS_UNIT_TEST( GamowInterpolator, processIndepVar )
{
  std::shared_ptr<const Utility::Interpolator<double> > interpolator =
    Utility::GamowInterpolator<double>::getExothermicInstance();

  TEST_FLOATING_EQUALITY( 0.1, interpolator->processIndepVar( 0.1 ), 1e-15 );
  TEST_FLOATING_EQUALITY( 1.0, interpolator->processIndepVar( 1.0 ), 1e-15 );

  interpolator.reset( new Utility::GamowInterpolator<double>( 1.0 ) );

  TEST_FLOATING_EQUALITY( 0.1, interpolator->processIndepVar( 1.1 ), 1e-15 );
  TEST_FLOATING_EQUALITY( 1.0, interpolator->processIndepVar( 2.0 ), 1e-15 );
}

//---------------------------------------------------------------------------//
// Check that an independent variable can be processed
TEUCHOS_UNIT_TEST( GamowUnitAwareInterpolator, processIndepVar )
{
  std::shared_ptr<const Utility::UnitAwareInterpolator<MegaElectronVolt,Barn,double> > interpolator = 
    Utility::GamowUnitAwareInterpolator<MegaElectronVolt,Barn,double>::getExothermicInstance();
...
TEUCHOS_UNIT_TEST( GamowInterpolator, processDepVar )
{
  std::shared_ptr<const Utility::Interpolator<double> > interpolator =
    Utility::GamowInterpolator<double>::getExothermicInstance();

  TEST_FLOATING_EQUALITY( 0.1, interpolator->processDepVar( 0.1 ), 1e-15 );
  TEST_FLOATING_EQUALITY( 1.0, interpolator->processDepVar( 1.0 ), 1e-15 );
...
//---------------------------------------------------------------------------//
// Check that a dependent variable can be processed
TEUCHOS_UNIT_TEST( GamowUnitAwareInterpolator, processDepVar )
{
  std::shared_ptr<const Utility::UnitAwareInterpolator<MegaElectronVolt,Barn,double> > interpolator = 
    Utility::GamowUnitAwareInterpolator<MegaElectronVolt,Barn,double>::getExothermicInstance();

  TEST_FLOATING_EQUALITY( 0.1, interpolator->processDepVar( 0.1*barn ), 1e-15);
  TEST_FLOATING_EQUALITY( 1.0, interpolator->processDepVar( 1.0*barn ), 1e-15);
...
TEUCHOS_UNIT_TEST( GamowInterpolator,recoverProcessedIndepVar )
{
  std::shared_ptr<const Utility::Interpolator<double> > interpolator =
    Utility::GamowInterpolator<double>::getExothermicInstance();
...
//---------------------------------------------------------------------------//
// Check that a processed independent variable can be recovered
TEUCHOS_UNIT_TEST( GamowUnitAwareInterpolator, recoverProcessedIndepVar )
{
  std::shared_ptr<const Utility::UnitAwareInterpolator<MegaElectronVolt,Barn,double> > interpolator = 
    Utility::GamowUnitAwareInterpolator<MegaElectronVolt,Barn,double>::getExothermicInstance();
...
TEUCHOS_UNIT_TEST( GamowInterpolator, recoverProcessedDepVar )
{
  std::shared_ptr<const Utility::Interpolator<double> > interpolator =
    Utility::GamowInterpolator<double>::getExothermicInstance();
...
//---------------------------------------------------------------------------//
// Check that a processed dependent variable can be recovered
TEUCHOS_UNIT_TEST( GamowUnitAwareInterpolator, recoverProcessedDepVar )
{
  std::shared_ptr<const Utility::UnitAwareInterpolator<MegaElectronVolt,Barn,double> > interpolator = 
    Utility::GamowUnitAwareInterpolator<MegaElectronVolt,Barn,double>::getExothermicInstance();
...
TEUCHOS_UNIT_TEST( GamowInterpolator, interpolate )
{
  std::shared_ptr<const Utility::Interpolator<double> > interpolator =
    Utility::GamowInterpolator<double>::getExothermicInstance();
  ...
  TEST_FLOATING_EQUALITY( y, 1093.9254005049686, 1e-12 );

  x = 0.1;

  y = interpolator->interpolate( x0, x1, x, y0, y1 );

  TEST_FLOATING_EQUALITY( y, 10.0, 1e-12 );

  x = 10.0;

  y = interpolator->interpolate( x0, x1, x, y0, y1 );

  TEST_FLOATING_EQUALITY( y, 1000.0, 1e-12 );
  ...
  TEST_FLOATING_EQUALITY( y, 2077.3469921631245, 1e-12 );
...
//---------------------------------------------------------------------------//
// Check that interpolation between two points can be done
TEUCHOS_UNIT_TEST( GamowUnitAwareInterpolator, interpolate )
{
  std::shared_ptr<const Utility::UnitAwareInterpolator<MegaElectronVolt,Barn,double> > interpolator = 
    Utility::GamowUnitAwareInterpolator<MegaElectronVolt,Barn,double>::getExothermicInstance();
  ...
  UTILITY_TEST_FLOATING_EQUALITY( cs, 1093.9254005049686*barns, 1e-12 );

  e = 0.1*MeV;

  cs = interpolator->interpolate( e0, e1, e, cs0, cs1 );

  UTILITY_TEST_FLOATING_EQUALITY( cs, 10.0*barn, 1e-12 );

  e = 10.0*MeV;

  cs = interpolator->interpolate( e0, e1, e, cs0, cs1 );

  UTILITY_TEST_FLOATING_EQUALITY( cs, 1000.0*barn, 1e-12 );
  ...
  UTILITY_TEST_FLOATING_EQUALITY( cs, 2077.3469921631245*barn, 1e-12 );
...
TEUCHOS_UNIT_TEST( GamowInterpolator, interpolateProcessed )
{
  std::shared_ptr<const Utility::Interpolator<double> > interpolator =
    Utility::GamowInterpolator<double>::getExothermicInstance();
  ...
  double b_value = log( (1000.0*10.0)/(10.0*0.1) )/
  ...
  b_value = log( (1000.0*10.0)/(10.0*0.1) )/
    ( 1.0/sqrt(processed_x0) - 1.0/sqrt(processed_x1) );
...
//---------------------------------------------------------------------------//
// Check that interpolation between two processed points can be done
TEUCHOS_UNIT_TEST( GamowUnitAwareInterpolator, interpolateProcessed )
{
  std::shared_ptr<const Utility::UnitAwareInterpolator<MegaElectronVolt,Barn,double> > interpolator = 
    Utility::GamowUnitAwareInterpolator<MegaElectronVolt,Barn,double>::getExothermicInstance();
  ...
  UTILITY_TEST_FLOATING_EQUALITY( cs, 10.0*barns, 1e-12 );
  ...
  UTILITY_TEST_FLOATING_EQUALITY( cs, 1000.0*barns, 1e-12 );
  ...
  UTILITY_TEST_FLOATING_EQUALITY( cs, 2077.3469921631245*barns, 1e-12 );
...
//---------------------------------------------------------------------------//
// Check that interpolation between two points can be done
TEUCHOS_UNIT_TEST( GamowInterpolator, interpolateAndProcess )
{
  std::shared_ptr<const Utility::Interpolator<double> > interpolator =
    Utility::GamowInterpolator<double>::getExothermicInstance();
  ...
  TEST_FLOATING_EQUALITY( processed_y, 1093.9254005049686, 1e-12 );

  x = 0.1;

  processed_y = interpolator->interpolateAndProcess( x0, x1, x, y0, y1 );

  TEST_FLOATING_EQUALITY( processed_y, 10.0, 1e-12 );

  x = 10.0;

  processed_y = interpolator->interpolateAndProcess( x0, x1, x, y0, y1 );

  TEST_FLOATING_EQUALITY( processed_y, 1000.0, 1e-12 );
  ...
  TEST_FLOATING_EQUALITY( processed_y, 2077.3469921631245, 1e-12 );
...
//---------------------------------------------------------------------------//
// Check that interpolation between two points can be done
TEUCHOS_UNIT_TEST( GamowUnitAwareInterpolator, interpolateAndProcess )
{
  std::shared_ptr<const Utility::UnitAwareInterpolator<MegaElectronVolt,Barn,double> > interpolator = 
    Utility::GamowUnitAwareInterpolator<MegaElectronVolt,Barn,double>::getExothermicInstance();
  ...
  UTILITY_TEST_FLOATING_EQUALITY( processed_cs, 1093.9254005049686, 1e-12 );

  e = 0.1*MeV;

  processed_cs = interpolator->interpolateAndProcess( e0, e1, e, cs0, cs1 );

  UTILITY_TEST_FLOATING_EQUALITY( processed_cs, 10.0, 1e-12 );

  e = 10.0*MeV;

  processed_cs = interpolator->interpolateAndProcess( e0, e1, e, cs0, cs1 );

  UTILITY_TEST_FLOATING_EQUALITY( processed_cs, 1000.0, 1e-12 );
  ...
  UTILITY_TEST_FLOATING_EQUALITY( processed_cs, 2077.3469921631245, 1e-12 );
...
//---------------------------------------------------------------------------//
// Check that interpolation between two processed points can be done
TEUCHOS_UNIT_TEST( GamowInterpolator, interpolateProcessedAndProcess )
{ 
  std::shared_ptr<const Utility::Interpolator<double> > interpolator =
    Utility::GamowInterpolator<double>::getExothermicInstance();
...
//---------------------------------------------------------------------------//
// Check that interpolation between two processed points can be done
TEUCHOS_UNIT_TEST( GamowUnitAwareInterpolator,
