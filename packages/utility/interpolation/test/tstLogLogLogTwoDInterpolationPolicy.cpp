...
UNIT_TEST_INSTANTIATION_1_TUPLE( LogLogLog, calculateGridLength );

//---------------------------------------------------------------------------//
// Check that the grid length can be calculated
TEUCHOS_UNIT_TEST( LogLogLog, calculateGridLength_no_tuple )
{
...
UNIT_TEST_INSTANTIATION_1_TUPLE( LogLogLog, calculateGridLengthProcessed );

//---------------------------------------------------------------------------//
// Check that the grid length can be calculated
TEUCHOS_UNIT_TEST( LogLogLog, calculateGridLengthProcessed_no_tuple )
{
  Teuchos::Array<double> grid( 4 );
  grid[0] = Utility::LogLogLog::processSecondIndepVar(1e-3);
  grid[1] = Utility::LogLogLog::processSecondIndepVar(1e-2);
...
//---------------------------------------------------------------------------//
// Check that the unit base independent can be calculated
TEUCHOS_UNIT_TEST( LogLogLog, calculateUnitBaseIndepVarProcessed )
{
  double y_min = Utility::LogLogLog::processSecondIndepVar(1e-3);
  double y = Utility::LogLogLog::processSecondIndepVar(1e-2);
  double L = 3.0;

  double eta = Utility::LogLogLog::calculateUnitBaseIndepVarProcessed(
								 y, y_min, L );
  ...
  eta = Utility::LogLogLog::calculateUnitBaseIndepVarProcessed( y, y_min, L );

  TEST_FLOATING_EQUALITY( eta, 0.0, 1e-12 );

  y = Utility::LogLogLog::processSecondIndepVar(0.020085536923187);
...
// Check that the grid independent y variable can be calculated given a
// unit base independent variable
TEUCHOS_UNIT_TEST( LogLogLog, calculateProcessedGridIndepVar )
{
  double y_min = Utility::LogLogLog::processSecondIndepVar(1e-3);
