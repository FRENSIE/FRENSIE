//---------------------------------------------------------------------------//
//!
//! \file   Utility_PolynomialDistribution.cpp
//! \author Alex Robinson
//! \brief  The polynomial distribution class template instantiations
//!
//---------------------------------------------------------------------------//

// FRENSIE Includes
#include "Utility_Archives.hpp"
#include "Utility_PolynomialDistribution.hpp"

EXPLICIT_TEMPLATE_CLASS_INST( Utility::UnitAwarePolynomialDistribution<void,void> );
EXPLICIT_CLASS_SAVE_LOAD_INST( Utility::UnitAwarePolynomialDistribution<void,void> );

//---------------------------------------------------------------------------//
// end Utility_PolynomialDistribution.cpp
//---------------------------------------------------------------------------//
