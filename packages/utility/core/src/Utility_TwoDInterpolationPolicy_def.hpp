...
template<typename ZYInterpPolicy, typename ZXInterpPolicy>
template<typename T>
inline T TwoDInterpolationPolicyImpl<ZYInterpPolicy,ZXInterpPolicy>::calculateUnitBaseIndepVarProcessed( 
			        const T processed_indep_var_y,
				...
				const T intermediate_grid_length )
{
  // Make sure the intermediate grid min indep var is valid
  testPrecondition( !Teuchos::ScalarTraits<T>::isnaninf( 
				    processed_intermediate_indep_var_y_min ) );
  // Make sure the independent y variable is valid
  testPrecondition( !Teuchos::ScalarTraits<T>::isnaninf( 
						     processed_indep_var_y ) );
  testPrecondition( processed_indep_var_y >= 
		    ...
		    Policy::relError(processed_indep_var_y,
				     processed_intermediate_indep_var_y_min) <=
		    s_tol );
  remember( double test_difference = processed_indep_var_y - 
	    processed_intermediate_indep_var_y_min );
  testPrecondition( test_difference <= intermediate_grid_length ||
		    Policy::relError( test_difference,
				      intermediate_grid_length ) <= s_tol );
  ...
  T eta = (processed_indep_var_y - processed_intermediate_indep_var_y_min)/
    intermediate_grid_length;
...
template<typename ZYInterpPolicy, typename ZXInterpPolicy>
template<typename T>
inline T TwoDInterpolationPolicyImpl<ZYInterpPolicy,ZXInterpPolicy>::calculateProcessedGridIndepVar( 
				  const T eta,
				  ...
				  const T grid_length )
{
  ...
  // Make sure the grid min indep var is valid
  testPrecondition( !Teuchos::ScalarTraits<T>::isnaninf( 
					      processed_grid_indep_var_min ) );
  // Make sure the grid length is valid
  testPrecondition( !Teuchos::ScalarTraits<T>::isnaninf( grid_length ) );
  testPrecondition( grid_length > 0.0 );
  
  return processed_grid_indep_var_min + grid_length*eta;
}
