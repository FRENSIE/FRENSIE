...
private:
  ...
  std::vector<std::pair<HexIndex,double>> traceThroughMesh( double point[3],
                                                               const double direction[3],
                                                               const double track_length,
                                                               PlaneIndex hex_plane_indices[3] ) const;

  // Find the interaction planes that a particle will interact with
  std::vector<std::pair<Dimension, PlaneIndex>> findInteractionPlanes( const double point[3],
                                                                          const double direction[3],
  ...
  std::pair<Dimension, double> findIntersectionDistance( const double point[3],
                                                         const double direction[3],
                                                         const std::vector<std::pair<Dimension, PlaneIndex>>& interaction_planes) const;
  ...
  std::pair<Dimension, double> findShortestDistance( const std::vector<std::pair<Dimension, double>>& distance_array ) const;
  ...
  std::vector<std::pair<Dimension,double>> findDistances( 
                                                        const double point[3],
                                                        const double direction[3],
                      const std::vector<std::pair<Dimension,PlaneIndex>>& planeSet ) const;
                      
  //used to find the relevant intersection point where the particle will intersect the mesh
  std::pair<Dimension,double> findIntersectionPoint( const std::vector<std::pair<Dimension,double>>& distances) const;
