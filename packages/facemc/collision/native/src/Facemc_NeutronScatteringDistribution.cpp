...
double NeutronScatteringDistribution::sampleScatteringAngleCosine( 
			const double energy,
			const AngularDistribution& angular_distribution ) const
{
  double angle_cosine;

  if( energy < angular_distribution.front().first )
  {
    angle_cosine = 
      angular_distribution.front().second->sample();
  }
  else if( energy >= angular_distribution.back().first )
  {
    angle_cosine = 
      angular_distribution.back().second->sample();
  }
  else
  {
    ...
    lower_bin_boundary = angular_distribution.begin();
    upper_bin_boundary = angular_distribution.end();
