						     ...
						     const double E )
{
  ...
  GaussKronrodQuadratureKernel::throwExceptions( true );
  ...
  if( alpha >= alpha_min && alpha <= alpha_max )
  {
    if( alpha > 0.0 )
    {
      boost::function<double (double mu_cm)> d_integrand = 
	boost::bind<double>( &FreeGasElasticSAlphaBetaFunction::evaluateIntegrand,
			     boost::ref( *this),
			     alpha,
			     beta,
			     E,
...
double FreeGasElasticScatteringKernelFactor::calculateExpArgConst( 
							    const double alpha,
							    const double beta,
							    const double E )
{
...
double FreeGasElasticScatteringKernelFactor::calculateExpArgMult( 
							   const double alpha )
{
...
double FreeGasElasticScatteringKernelFactor::calculateBesselArgMult( 
							    const double alpha,
							    const double beta,
							    const double E )
{
						    ...
						    double& upper_limit ) const
{
  ...
  if( estimated_peak_exp_arg > min_exp_arg )
  {
    ...
    double center_value = 
      this->findCMScatteringAngleCosineWithNonZeroFunctionValue( alpha,
								 beta,
								 E,
								 search_grid );
...
{  
  ...
  return findCMScatteringAngleCosineWithNonZeroFunctionValue( alpha,
							      beta,
							      E,
							      grid_points );
}
