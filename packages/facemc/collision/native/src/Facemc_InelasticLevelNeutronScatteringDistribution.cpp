...
{
    // Check to make sure distribution has atleast one bin
                                                                 ...
                                                                 const double temperature ) const
{
   ...
   double cm_scatter_angle_cosine = sampleScatteringAngleCosine( neutron.getEnergy(),
   ...
   double energy = cm_energy + ( neutron.getEnergy() + 2.0 * cm_scatter_angle_cosine * ( atom_ratio_plus_1 ) *
                   sqrt( neutron.getEnergy() * cm_energy ) ) / ( atom_ratio_plus_1 * atom_ratio_plus_1 );

   // Calculate the scattering angle for the lab frame
   double scatter_angle_cosine = cm_scatter_angle_cosine * sqrt( cm_energy / energy ) +
   ...
   Utility::rotateDirectionThroughPolarAndAzimuthalAngle( scatter_angle_cosine,
