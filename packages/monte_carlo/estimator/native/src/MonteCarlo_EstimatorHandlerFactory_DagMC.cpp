//---------------------------------------------------------------------------//
//!
//! \file   MonteCarlo_EstimatorHandlerFactory_DagMC.cpp
				 ...
				 std::ostream& os_warn )
{
  ...
  while( it != estimator_reps.end() )
  {
    ...
    EstimatorHandlerFactory<moab::DagMC>::validateEstimatorRep( estimator_rep,
						   response_id_map );
  ...
  EstimatorHandlerFactory<moab::DagMC>::createEstimatorDataMaps(
  ...
  EstimatorHandlerFactory<moab::DagMC>::appendDataToEstimatorDataMaps(
						   estimator_reps,
						   estimator_id_type_map,
						   estimator_id_ptype_map,
						   estimator_id_cells_map,
  ...
  EstimatorHandlerFactory<moab::DagMC>::createCellVolumeMap( estimator_id_cells_map,
						cell_volume_map );
  ...
  EstimatorHandlerFactory<moab::DagMC>::createSurfaceAreaMap( estimator_id_surfaces_map,
						 surface_area_map );
  ...
  while( it != estimator_reps.end() )
  {
	...
	estimator_id_cells_map[id];
      ...
      if( EstimatorHandlerFactory<moab::DagMC>::isCellPulseHeightEstimator( estimator_id_type_map[id] ) )
      {
	EstimatorHandlerFactory<moab::DagMC>::createPulseHeightEstimator(
							id,
							multiplier,
							particle_types,
							cells,
      ...
      else if( EstimatorHandlerFactory<moab::DagMC>::isCellTrackLengthFluxEstimator( estimator_id_type_map[id] ) )
      {
	EstimatorHandlerFactory<moab::DagMC>::fillCellVolumesArray( cells,
						       cell_volume_map,
						       cell_volumes );
	
	EstimatorHandlerFactory<moab::DagMC>::createCellTrackLengthFluxEstimator(
							id,
							multiplier,
							particle_types,
							cells,
      ...
      else if( EstimatorHandlerFactory<moab::DagMC>::isCellCollisionFluxEstimator( estimator_id_type_map[id] ) )
      {
	EstimatorHandlerFactory<moab::DagMC>::fillCellVolumesArray( cells,
						       cell_volume_map,
						       cell_volumes );
	
	EstimatorHandlerFactory<moab::DagMC>::createCellCollisionFluxEstimator(
							id,
							multiplier,
							particle_types,
							cells,
	...
	surfaces = estimator_id_surfaces_map[id];
      ...
      if( EstimatorHandlerFactory<moab::DagMC>::isSurfaceFluxEstimator( estimator_id_type_map[id] ) )
      {
	EstimatorHandlerFactory<moab::DagMC>::fillSurfaceAreasArray( surfaces,
							surface_area_map,
							surface_areas );
	
	EstimatorHandlerFactory<moab::DagMC>::createSurfaceFluxEstimator(
							id,
							multiplier,
							particle_types,
							surfaces,
      ...
      {	
	EstimatorHandlerFactory<moab::DagMC>::createSurfaceCurrentEstimator(
							id,
							multiplier,
							particle_types,
							surfaces,
    ...
    // Create a tet mesh track length flux estimator
	     ...
	     EstimatorHandlerFactory<moab::DagMC>::tet_mesh_track_length_flux_name )
    {
      ...
      EstimatorHandlerFactory<moab::DagMC>::createTetMeshTrackLengthFluxEstimator(
							 id,
							 multiplier,
							 particle_types,
							 response_functions,
...
void EstimatorHandlerFactory<moab::DagMC>::validateEstimatorRep(
	  const Teuchos::ParameterList& estimator_rep,
...
bool EstimatorHandlerFactory<moab::DagMC>::areEstimatorTypesEquivalent( 
						 ...
						 const std::string& xml_type )
{
  ...
  else
  {
    ...
    {
      if( xml_type == EstimatorHandlerFactory<moab::DagMC>::surface_current_name )
	return true;
      else
	return false;
    }
    ...
    {
      if( xml_type == EstimatorHandlerFactory<moab::DagMC>::surface_flux_name )
	return true;
      else
	return false;
    }
    ...
    {
      if( xml_type == EstimatorHandlerFactory<moab::DagMC>::cell_pulse_height_name )
	return true;
      else
	return false;
    }
    ...
    {
      if( xml_type == EstimatorHandlerFactory<moab::DagMC>::cell_track_length_flux_name )
	return true;
      else
	return false;
    }
    ...
    {
      if( xml_type == EstimatorHandlerFactory<moab::DagMC>::cell_collision_flux_name )
	return true;
      else
	return false;
    }
...
void EstimatorHandlerFactory<moab::DagMC>::createEstimatorDataMaps(
	boost::unordered_map<unsigned,std::string>& estimator_id_type_map,
	boost::unordered_map<unsigned,std::string>& estimator_id_ptype_map,
	boost::unordered_map<unsigned,
	          Teuchos::Array<Geometry::ModuleTraits::InternalCellHandle> >&
...
// If DagMC is not enabled this function will be empty
void EstimatorHandlerFactory<moab::DagMC>::appendDataToEstimatorDataMaps(
	const Teuchos::ParameterList& estimator_reps,
	boost::unordered_map<unsigned,std::string>& estimator_id_type_map,
	boost::unordered_map<unsigned,std::string>& estimator_id_ptype_map,
	boost::unordered_map<unsigned,
	...
	estimator_id_surfaces_map )
{
  ...
  TEST_FOR_EXCEPTION( !areEstimatorTypesEquivalent( estimator_id_type_map.find( id )->second,
	...
	if( estimator_rep.isParameter( "Cells" ) )
	{
	  ...
	  EstimatorHandlerFactory<moab::DagMC>::appendCellsToAssignedCells( 
						    id,
				                    estimator_id_cells_map[id],
						    extra_cells );
	}
	...
	if( estimator_rep.isParameter( "Surfaces" ) )
	{
	  ...
	  EstimatorHandlerFactory<moab::DagMC>::appendSurfacesToAssignedSurfaces(
						 id,
    ...
    else
    {
      ...
      TEST_FOR_EXCEPTION( 
       !Geometry::DagMCProperties::isCellEstimatorTypeValid(estimator_type) &&
       !Geometry::DagMCProperties::isSurfaceEstimatorTypeValid(estimator_type) &&
       !EstimatorHandlerFactory<moab::DagMC>::isEstimatorTypeValid(estimator_type),
       InvalidEstimatorRepresentation,
	...
	estimator_rep.get<std::string>( "Particle Type" );
      ...
      if( Geometry::DagMCProperties::isCellEstimatorTypeValid(estimator_type)||
          EstimatorHandlerFactory<moab::DagMC>::isCellEstimatorTypeValid(estimator_type) )
      {
	...
	EstimatorHandlerFactory<moab::DagMC>::appendCellsToAssignedCells( 
               ...
               EstimatorHandlerFactory<moab::DagMC>::isSurfaceEstimatorTypeValid( estimator_type ) )
      {
...
// If DagMC is not enabled, this function will be empty
void EstimatorHandlerFactory<moab::DagMC>::appendCellsToAssignedCells(
...
// If DagMC is not enabled, this function will be empty
void EstimatorHandlerFactory<moab::DagMC>::appendSurfacesToAssignedSurfaces(
	   const unsigned estimator_id,
	   Teuchos::Array<Geometry::ModuleTraits::InternalSurfaceHandle>&
	   assigned_surfaces,
...
// Create the cell volume map
// If DagMC is not enabled, this function will be empty
void EstimatorHandlerFactory<moab::DagMC>::createCellVolumeMap(
...
// Create the surface area map
// If DagMC is not enabled, this function will be empty
void EstimatorHandlerFactory<moab::DagMC>::createSurfaceAreaMap(
    const boost::unordered_map<unsigned,
               Teuchos::Array<Geometry::ModuleTraits::InternalSurfaceHandle> >&
    estimator_id_surfaces_map,
    boost::unordered_map<Geometry::ModuleTraits::InternalSurfaceHandle,double>&
...
void EstimatorHandlerFactory<moab::DagMC>::createPulseHeightEstimator(
      const unsigned id,
      ...
      const Teuchos::ParameterList* bins )
{
  ...
  if( bins )
    EstimatorHandlerFactory<moab::DagMC>::assignBinsToEstimator( *bins, estimator );
...
void EstimatorHandlerFactory<moab::DagMC>::createCellTrackLengthFluxEstimator(
      const unsigned id,
      const double multiplier,
      const Teuchos::Array<ParticleType> particle_types,
      const Teuchos::Array<Geometry::ModuleTraits::InternalCellHandle>& cells,
...
void EstimatorHandlerFactory<moab::DagMC>::createCellCollisionFluxEstimator(
      const unsigned id,
      const double multiplier,
      const Teuchos::Array<ParticleType> particle_types,
      const Teuchos::Array<Geometry::ModuleTraits::InternalCellHandle>& cells,
...
void EstimatorHandlerFactory<moab::DagMC>::createSurfaceFluxEstimator(
      const unsigned id,
      const double multiplier,
      const Teuchos::Array<ParticleType> particle_types,
...
void EstimatorHandlerFactory<moab::DagMC>::createSurfaceCurrentEstimator(
      const unsigned id,
      const double multiplier,
      const Teuchos::Array<ParticleType> particle_types,
      const Teuchos::Array<Geometry::ModuleTraits::InternalSurfaceHandle>&
...
// Create a tet mesh track length flux estimator
void EstimatorHandlerFactory<moab::DagMC>::createTetMeshTrackLengthFluxEstimator(
	 const unsigned id,
	 const double multiplier,
	 const Teuchos::Array<ParticleType> particle_types,
	 const Teuchos::Array<Teuchos::RCP<ResponseFunction> >& response_funcs,
...
void EstimatorHandlerFactory<moab::DagMC>::assignBinsToEstimator( 
					   const Teuchos::ParameterList& bins,
					   Teuchos::RCP<Estimator>& estimator )
{
...
void EstimatorHandlerFactory<moab::DagMC>::fillCellVolumesArray( 
       const Teuchos::Array<Geometry::ModuleTraits::InternalCellHandle>& cells,
...
void EstimatorHandlerFactory<moab::DagMC>::fillSurfaceAreasArray(
      const Teuchos::Array<Geometry::ModuleTraits::InternalSurfaceHandle>&
      surfaces,
...
bool EstimatorHandlerFactory<moab::DagMC>::isEstimatorTypeValid(
					   const std::string& estimator_type )
{
  return EstimatorHandlerFactory<moab::DagMC>::isCellEstimatorTypeValid( estimator_type ) ||
    EstimatorHandlerFactory<moab::DagMC>::isSurfaceEstimatorTypeValid( estimator_type ) ||
    EstimatorHandlerFactory<moab::DagMC>::isMeshEstimatorTypeValid( estimator_type );
  
}

// Check if a cell estimator type is valid
bool EstimatorHandlerFactory<moab::DagMC>::isCellEstimatorTypeValid( 
					    const std::string& estimator_type )
{
  if( estimator_type == EstimatorHandlerFactory<moab::DagMC>::cell_track_length_flux_name )
    return true;
  else if( estimator_type == EstimatorHandlerFactory<moab::DagMC>::cell_collision_flux_name )
    return true;
  else if( estimator_type == EstimatorHandlerFactory<moab::DagMC>::cell_pulse_height_name )
    return true;
  else
    return false;
}
...
bool EstimatorHandlerFactory<moab::DagMC>::isSurfaceEstimatorTypeValid( 
					    const std::string& estimator_type )
{
  if( estimator_type == EstimatorHandlerFactory<moab::DagMC>::surface_current_name )
    return true;
  else if( estimator_type == EstimatorHandlerFactory<moab::DagMC>::surface_flux_name )
    return true;
  else
    return false;
}
...
bool EstimatorHandlerFactory<moab::DagMC>::isMeshEstimatorTypeValid( 
                                            const std::string& estimator_type )
{
  if( estimator_type == EstimatorHandlerFactory<moab::DagMC>::tet_mesh_track_length_flux_name )
    return true;
  else
    return false;
}
...
//---------------------------------------------------------------------------//
// end MonteCarlo_EstimatorHandlerFactory_DagMC.cpp
//---------------------------------------------------------------------------//
