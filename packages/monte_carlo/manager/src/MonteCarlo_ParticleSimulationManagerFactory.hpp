...
public:
  ...
  void setPopulationControllers( const std::vector<std::shared_ptr<const PopulationControl>>& population_controllers );
...
private:
  ...
  // The weight windows
  std::shared_ptr<const WeightWindow> d_population_controllers;
...
void ParticleSimulationManagerFactory::serialize( Archive& ar, const unsigned version )
{
  ...
  ar & BOOST_SERIALIZATION_NVP( d_population_controllers );
