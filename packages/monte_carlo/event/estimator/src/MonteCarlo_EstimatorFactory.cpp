                                  ...
                                  const Teuchos::ParameterList& estimator_rep )
{
  ...
  try{
    ...
    {
      // Get the cells
      std::vector<Geometry::ModuleTraits::InternalCellHandle> cells;
    ...
    {
      // Get the surfaces
      std::vector<Geometry::ModuleTraits::InternalSurfaceHandle> surfaces;
                 ...
                 const Teuchos::ParameterList& estimator_rep ) const
{
  ...
  if( estimator_rep.isParameter( "Surfaces" ) )
  {
    ...
    std::vector<unsigned> xml_surfaces;
        ...
        const Teuchos::ParameterList& estimator_rep ) const
{
  if( estimator_rep.isParameter( "Response Functions" ) )
  {
    ...
    std::vector<unsigned> requested_response_functions;
                                  ...
                                  std::shared_ptr<Estimator>& estimator ) const
{
  ...
  while( it != bins.end() )
  {
    ...
    {
      const Utility::ArrayString& array_string =
        boost::any_cast<Utility::ArrayString>( it->second.getAny() );

      std::vector<double> source_energy_bins;
	...
	boost::any_cast<Utility::ArrayString>( it->second.getAny() );
      ...
      std::vector<double> source_time_bins;
	...
	boost::any_cast<Utility::ArrayString>( it->second.getAny() );
      ...
      std::vector<unsigned> col_num_bins;
