    ...
    d_unfilled_model( geometry_model )
{
  ...
  {
    if( required_material_ids.find( *database_material_id_it ) ==
  ...
  geometry_model->getCellIdDensityMap( cell_id_density_map );
  ...
  if( MonteCarlo::isParticleTypeCompatible( mode, NEUTRON ) )
  {
    try{
      FilledNeutronGeometryModel::loadMaterialsAndFillModel(
                                                ...
                                                atomi_relaxation_model_factory,
                                                properties,
             ...
             const MonteCarlo::ParticleType particle_type )
{
  ...
  {
  case NEUTRON:
    return NeutronCollisionKernel::isCellVoid( cell );
  case PHOTON:
    return PhotonCollisionKernel::isCellVoid( cell );
  case ADJOINT_PHOTON:
    return AdjointPhotonCollisionKernel::isCellVoid( cell );
  case ELECTRON:
    return ElectronCollisionKernel::isCellVoid( cell );
  case ADJOINT_ELECTRON:
    return AdjointElectronCollisionKernel::isCellVoid( cell );
  case POSITRON:
    return PositronCollisionKernel::isCellVoid( cell );
  default:
