        ...
        const double scattering_angle_cosine ) const
{
  ...
  long double test = static_cast<long double>(scattering_angle_cosine);
  double delta_mu = 1.0L - test;
                                                ...
                                                unsigned& trials ) const
{
  ...
  if ( d_using_endl_tables )
  {
    ...
    double lower_delta_mu = ( lower_bin_boundary->second*lower_bin_boundary->second 
                          *scaled_random_number )/(
                        lower_bin_boundary->third -
                        lower_bin_boundary->second*scaled_random_number );

    double upper_delta_mu = ( upper_bin_boundary->second*upper_bin_boundary->second 
                          *scaled_random_number )/(
