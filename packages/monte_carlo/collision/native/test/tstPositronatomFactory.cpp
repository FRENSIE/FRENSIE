...
TEUCHOS_UNIT_TEST( PositronatomFactory, createPositronatomMap_ace_basic )
{
  // Create the set of positron-atom aliases
  std::unordered_set<std::string> positronatom_aliases;
  ...
  TEST_EQUALITY_CONST( positronatom_map.size(), 1 );
  TEST_ASSERT( positronatom_map.count( "Pb" ) );
  TEST_ASSERT( !positronatom_map["Pb"].is_null() );

  Teuchos::RCP<MonteCarlo::Positronatom>& atom = positronatom_map["Pb"];
...
TEUCHOS_UNIT_TEST( PositronatomFactory, createPositronatomMap_epr14 )
{
  // Create the set of positron-atom aliases
  std::unordered_set<std::string> positronatom_aliases;
  ...
  TEST_EQUALITY_CONST( positronatom_map.size(), 1 );
  TEST_ASSERT( positronatom_map.count( "Pb_v14" ) );
  TEST_ASSERT( !positronatom_map["Pb_v14"].is_null() );

  Teuchos::RCP<MonteCarlo::Positronatom>& atom = positronatom_map["Pb_v14"];
...
TEUCHOS_UNIT_TEST( PositronatomFactory, createPositronatomMap_hybrid )
{
  ...
  Teuchos::RCP<Data::ElectronPhotonRelaxationDataContainer> data_container(
    new Data::ElectronPhotonRelaxationDataContainer( positronatom_file_path ) );

  std::shared_ptr<const MonteCarlo::CutoffElasticElectronScatteringDistribution>
    elastic_distribution;

  MonteCarlo::ElasticElectronScatteringDistributionNativeFactory::createCutoffElasticDistribution<Utility::UnitBaseCorrelated<Utility::LogLogCosLog<false> > >(
        elastic_distribution,
        *data_container,
  ...
  TEST_EQUALITY_CONST( positronatom_map.size(), 1 );
  TEST_ASSERT( positronatom_map.count( "Pb-Native" ) );
  TEST_ASSERT( !positronatom_map["Pb-Native"].is_null() );

  Teuchos::RCP<MonteCarlo::Positronatom>& atom = positronatom_map["Pb-Native"];
...
TEUCHOS_UNIT_TEST( PositronatomFactory, createPositronatomMap_ace_2BS_brem )
{
  // Create the set of positron-atom aliases
  std::unordered_set<std::string> positronatom_aliases;
  positronatom_aliases.insert( "Pb" );
  ...
  cross_section = atom->getReactionCrossSection( 2.0e-3, reaction );
  TEST_FLOATING_EQUALITY( cross_section, 1.77467265616e7, 1e-12 );
...
//---------------------------------------------------------------------------//
// Check that a positron-atom map can be created (2BS bremsstrahlung data)
TEUCHOS_UNIT_TEST( PositronatomFactory, createPositronatomMap_native_2BS_brem )
{
  // Create the set of positron-atom aliases
  std::unordered_set<std::string> positronatom_aliases;
  positronatom_aliases.insert( "Pb-Native" );
...
TEUCHOS_UNIT_TEST( PositronatomFactory, createPositronatomMap_ace_ionization_subshells )
{
  // Create the set of positron-atom aliases
  std::unordered_set<std::string> positronatom_aliases;
  positronatom_aliases.insert( "Pb" );
...
//---------------------------------------------------------------------------//
// Check that a positron-atom map can be created (positron-ionization subshell data)
TEUCHOS_UNIT_TEST( PositronatomFactory, createPositronatomMap_native_ionization_subshells )
{
  // Create the set of positron-atom aliases
  std::unordered_set<std::string> positronatom_aliases;
  positronatom_aliases.insert( "Pb-Native" );
...
TEUCHOS_UNIT_TEST( PositronatomFactory, no_duplicate_tables )
{
  // Create the set of positron-atom aliases
  std::unordered_set<std::string> positronatom_aliases;
  ...
  TEST_EQUALITY_CONST( positronatom_map.size(), 2 );

  TEST_ASSERT( positronatom_map.count( "H-1_293.6K" ) );
  TEST_ASSERT( !positronatom_map["H-1_293.6K"].is_null() );

  TEST_ASSERT( positronatom_map.count( "H-1_300K" ) );
  TEST_ASSERT( !positronatom_map["H-1_300K"].is_null() );

  TEST_EQUALITY( positronatom_map["H-1_293.6K"], positronatom_map["H-1_300K"] );
}
