...
TEUCHOS_UNIT_TEST( ModuleInterface, burn )
{
   ...
   Transmutation::ModuleInterface<Utility::BoostSolver>::setNumberDensities( number_density_map );
   ...
   // Set reaction rates
   Transmutation::ModuleInterface<Utility::BoostSolver>::setReactionRates( MonteCarlo::N__GAMMA_REACTION, reaction_rates, 1);

   reaction_rates[0] = std::pair<int,double>(1002,5e-10);

   Transmutation::ModuleInterface<Utility::BoostSolver>::setReactionRates( MonteCarlo::N__GAMMA_REACTION, reaction_rates, 5);

   number_density_map.clear();

   // Burn the materials
   Transmutation::ModuleInterface<Utility::BoostSolver>::burn( 1000000000.0, number_density_map );   
