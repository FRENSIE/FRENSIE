...
private:
  ...
  static double getLowerLogEnergyLimit( const double log_max_energy ) const;

  // Get the upper log energy limit for a given log max energy
  static double getUpperLogEnergyLimit( const double log_max_energy ) const;
  ...
  Teuchos::RCP<Utility::GaussKronrodQuadratureKernel> d_quadrature_kernel;
};
