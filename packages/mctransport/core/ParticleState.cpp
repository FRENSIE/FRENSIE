...
// Constructor
  ...
  : PrintableObject( "ParticleState" ),
    d_core( history_number, 
	    ...
	    0u,
	    1.0 ),
    d_cell( Traits::ModuleTraits::invalid_internal_cell_handle ),
    d_lost( false ),
    d_gone( false )
{ /* ... */ }
...
Traits::ModuleTraits::InternalCellHandle ParticleState::getCell() const
{
  return d_cell;
}
...
void ParticleState::setCell( 
			  const Traits::ModuleTraits::InternalCellHandle cell )
{
  // Make sure the cell handle is valid
  testPrecondition(cell != Traits::ModuleTraits::invalid_internal_cell_handle);
