//---------------------------------------------------------------------------//
//!
//! \file   YieldBasePhotonProductionReaction.cpp
...
// FACEMC Includes
#include "YieldBasedPhotonProductionReaction.hpp"
    ...
    d_base_reaction( base_reaction )
{
  ...
  testPrecondition( Sort::isSortedAscending( yield_energy_grid.begin(),
						    ...
						    const double energy ) const
{
      ...
      energy < d_yield_energy_grid.back() )
  {
    unsigned yield_index = 
      Search::binaryLowerBoundIndex( d_yield_energy_grid.begin(),
				     d_yield_energy_grid.end(),
				     energy );

    double yield =
      LinLin::interpolate( d_yield_energy_grid[yield_index],
...
//---------------------------------------------------------------------------//
// end YieldBasedPhotonProductionReaction.cpp
//---------------------------------------------------------------------------//
