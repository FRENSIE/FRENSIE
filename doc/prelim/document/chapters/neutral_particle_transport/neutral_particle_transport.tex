...
While this averaging of the transfer probabilities is necessary in the context 
of the transport equation, it is not necessary in a Monte Carlo random walk 
process. Instead, one can use detailed transfer probabilities and conduct 
...
analogue random walk process for neutral particles is impossible in a system
...
$p_{A,j,k}(E^{'})$ restrict the number of particles emitted to integer values.
...
\begin{figure}[t!]
  \begin{center}
    \scalebox{0.75}{\includegraphics{chapters/neutral_particle_transport/random_walk_process.pdf}}
    \end{center}
...
It must be mentioned that the particle flux can also be estimated indirectly
